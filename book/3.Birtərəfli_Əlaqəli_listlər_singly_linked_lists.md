...
| Ortadan silmə   | O(n)    | O(n), əgər massiv tam dolu deyilsə(elementləri sürüşdürmək üçün) | O(n)
...
Daxil etmə(insert) əməliyyatlarındakı kimi silmə üçün də 3 növ əməliyyat var:
...
* Burada da, həm əvvəlki, həm də hal-hazırkı node-u(silmək istədiyimiz) yadda saxlamalıyıq. Yəni, listi qət edərkən, əgər axtardığımız node tapılıbsa, bu zaman ondan bir əvvəlki node-un next pointer-ini, axtarılan node-un(silinəcək olan node-un) next pointer-inə yönləndiririk.
