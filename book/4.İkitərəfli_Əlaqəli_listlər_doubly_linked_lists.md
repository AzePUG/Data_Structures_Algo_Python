...
public static void main(String[] args){
   ...
   dll.insert_at_beginning(88);
   dll.list_size(); // Nəticə 3
   
   dll.delete_first_node();
   dll.list_size(); // Nəticə 2
   
   dll.delete_first_node();
   dll.list_size(); // Nəticə 1
...
public void delete_last_node(){
        if(list_size() == 0) System.out.println("List boşdur...");
            ...
            // Sonuncu node-dan bir əvvəlki node-u tapırıq
            Node previous_node = current_node.get_prev_node();
            // Bir əvvəlki node-un növbəti
...
```
// Java kod
public void traverse_and_print(){
        if(list_size() == 0) System.out.println("List boşdur...");
...
public static void main(String[] args){
   ...
   dll.insert_at_beginning(44);
   dll.insert_at_beginning(12);
   dll.insert_at_beginning(99);
   dll.insert_at_pos(2, 51); // Verilmiş pozisiyaya daxil etmə...
   dll.insert_at_end(101); 
...
```
// Java kod
public void delete_form_list_by_data(int data){
        // Burada, current, tapılan node-u, data-ya əsasən silirik.
        Node current_node  = head;
        ...
        while(current_node != null && found == false){
            ...
            else current_node = current_node.get_next_node();
        }
        // Fərqlilik məqsədilə Exception-dan istifadə edirik
        if(current_node == null) throw new IllegalArgumentException("Data listdə tapılmadı...");
        
        if(current_node.get_prev_node() == null){
            // Bu o deməkdir ki, axtarılan data elə 1ci(head) node-da tapılıb.
            // Bu zaman head-i sadəcə növbəti node-a işarəliyirik.
            // Əslində bu hal, list-in əvvəlindən node silməyə bərabərdir.
            head = current_node.get_next_node();
        }else if(current_node.get_next_node() == null){
            // Əgər axtarılan data sonuncudusa aşağıdakı proseslər baş verəcək
            Node current_previous_node = current_node.get_prev_node();
            current_previous_node.set_next_node(current_node.get_next_node());
        }else{
            // Əvvəlki node-un pointerini, hal-hazırkı(current) node-un next node-una yönəldirik.
            Node current_previous_node = current_node.get_prev_node();
            current_previous_node.set_next_node(current_node.get_next_node());
            // Sonrakı node-un əvvəlki pointerini isə hal-hazırkı(current) node-un əvvəli node-una yönəldirik.
            Node current_next_node = current_node.get_next_node();
            current_next_node.set_prev_node(current_node.get_prev_node());
        }
    }
```
...
public static void main(String[] args) {
        ...
        dll.insert_at_beginning(12);
        dll.insert_at_beginning(99);
        dll.insert_at_post(2, 51);
        dll.insert_at_end(101);
        dll.traverse_and_print();
        dll.delete_form_list_by_data(44);
        dll.traverse_and_print();
    }
...
public void delete_at_pos(int pos){
        ...
        if(pos > list_size() || pos < 0) {
            throw new IllegalArgumentException("Pozisiya səhvdir, null qaytarıram...");
        }else if(pos == 0){
            // Bu o demıkdir ki, listin əvvəlindən node silirik.
            delete_first_node();
        }else if(pos == list_size()){
            // Bu o demıkdir ki, listin sonundan node silirik.
            delete_last_node();
        }else{
            // Əgər listin axırına çatmamışıqsa və count verilmiş pozisiyadan kiçikdirsə, davam elə
...
public static void main(String[] args) {
        ...
        dll.insert_at_end(101);
        dll.traverse_and_print();
        dll.delete_at_pos(3); // 3 cü pozisiyya list 0 dan sayılmağa başladığı üçün 4cü data ya uyğun gəlir array larda olduğu kimi 
        dll.traverse_and_print();
    }
```
